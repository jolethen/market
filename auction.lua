...
-- UPGRADED: Expiry Logic (Handles Winners and Sellers)
local function check_expiry()
    ...
    for id, entry in pairs(ah) do
        if now >= entry.end_time then
            ...
            -- Add item to winner's reclaim bin
            ...
            -- If there was a bidder, the seller needs to receive their money
            if entry.last_bidder then
                local seller_storage = minetest.deserialize(eco_trade.storage:get_string("expired:" .. entry.seller)) or {}
                -- We store the money as a special string "MONEY:amount" to process during reclaim
    ...
    minetest.after(300, check_expiry) -- Check every 5 mins
end
minetest.after(300, check_expiry)

-- NEW: The Bidding Popup Formspec
function eco_trade.show_bid_window(player_name, auction_id)
    local ah = get_ah_data()
    local entry = ah[auction_id]
    if not entry then return end

    local min_bid = entry.price + entry.increment
    local fs = "size[6,5]background[0,0;6,5;eco_bg.png;true]" ..
               "label[0.5,0.5;Item: " .. entry.itemstring .. "]" ..
               "label[0.5,1.2;Current Bid: " .. entry.price .. "¢]" ..
               "label[0.5,1.6;Min Increment: " .. entry.increment .. "¢]" ..
               "label[0.5,2.0;Auto-Buy Price: " .. entry.buy_now .. "¢]" ..
               "field[0.8,3.2;4.5,1;bid_amount;Enter Bid (Min " .. min_bid .. "):;]" ..
...
function eco_trade.show_ah(player_name, page)
    ...
    for _, stack in ipairs(inv:get_list("main")) do
        if stack:get_name():match("currency:minegeld") then
            local value = tonumber(stack:get_name():match("_(%d+)$")) or 1
    ...
    local fs = "size[10,11]background[0,0;10,11;eco_bg.png;true]" ..
               "label[3.2,0.2;--- AUCTION HOUSE (Page " .. page .. "/" .. total_pages .. ") ---]"
    ...
    for i = start_idx, end_idx do
        ...
        if entry.seller == player_name then
            ...
            fs = fs .. ("label[%f,%f;Bid: %d¢]"):format(x, y + 1.7, entry.price)
        else
            fs = fs .. ("label[%f,%f;Bid: %d¢]"):format(x, y + 1.3, entry.price)
            fs = fs .. ("label[%f,%f;Buy: %d¢]"):format(x, y + 1.7, entry.buy_now)
            fs = fs .. ("label[%f,%f;%dh left]"):format(x, y + 2.1, hours_left)
        end
        
        x = x + 2.2
        if x > 8.5 then x = 0.8 y = y + 3.0 end
    end
    ...
    fs = fs .. ("label[0.3,10.6;Wallet: %d¢]"):format(balance)
